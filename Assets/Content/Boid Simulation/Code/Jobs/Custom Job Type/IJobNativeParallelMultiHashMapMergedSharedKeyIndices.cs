...
public static class JobNativeParallelMultiHashMapUniqueHashExtensions
{
        ...
        where T : struct, IJobNativeParallelMultiHashMapMergedSharedKeyIndices
    {
        ...
        return JobsUtility.ScheduleParallelFor(ref scheduleParams, hashMap.GetUnsafeBucketData().bucketCapacityMask + 1,
    ...
    internal struct JobNativeParallelMultiHashMapMergedSharedKeyIndicesProducer<T>
        where T : struct, IJobNativeParallelMultiHashMapMergedSharedKeyIndices
    {
        internal static readonly SharedStatic<IntPtr> reflectionData = SharedStatic<IntPtr>
            .GetOrCreate<JobNativeParallelMultiHashMapMergedSharedKeyIndicesProducer<T>>();
        ...
        internal static void Initialize()
        {
            if (reflectionData.Data == IntPtr.Zero)
                reflectionData.Data =
                    JobsUtility.CreateJobReflectionData(typeof(JobWrapper<T>), typeof(T), (ExecuteJobFunction)Execute);
        }

        delegate void ExecuteJobFunction(ref JobWrapper<T> jobWrapper, IntPtr additionalPtr,
            IntPtr bufferRangePatchData,
            ref JobRanges ranges, int jobIndex);

        [BurstCompile]
        public static unsafe void Execute(ref JobWrapper<T> jobWrapper, IntPtr additionalPtr,
            IntPtr bufferRangePatchData,
            ref JobRanges ranges, int jobIndex)
        {
            while (true)
                    ...
                    while (entryIndex != -1)
                    {
                        ...
                        if (entryIndex == it.GetEntryIndex())
                        {
                            ...
                            JobsUtility.PatchBufferMinMaxRanges(bufferRangePatchData,
                                UnsafeUtility.AddressOf(ref jobWrapper), value, 1);
#endif
                        ...
                        else
                        {
                            ...
                            JobsUtility.PatchBufferMinMaxRanges(bufferRangePatchData,
                                UnsafeUtility.AddressOf(ref jobWrapper), startIndex, rangeLength);
#endif
